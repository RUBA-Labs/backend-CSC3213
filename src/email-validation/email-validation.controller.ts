...
export class EmailValidationController {
    ...
    @ApiBody({
        ...
        examples: { a: { value: { email: 'test@example.com' } } },
    })
    async sendOtp(
    ...
    @ApiResponse({
        status: 200,
        description: 'Email is validated successfully.',
        schema: { example: { message: 'Email is validated successfully.' } },
    })
    @ApiResponse({
        status: 400,
        description: 'Invalid OTP or secret, or OTP expired.',
    })
    @ApiBody({
        type: VerifyOtpDto,
        examples: {
    ...
    async verifyOtp(@Body() verifyOtpDto: VerifyOtpDto, @Res() res: Response) {
        ...
        const result = await this.emailValidationService.verifyOtp(
            email,
            otp,
            secret,
        );
