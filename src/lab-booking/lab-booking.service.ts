...
export class LabBookingService {
  ...
  async getLabSessionBookingStatus(labSessionId: string): Promise<any> {
    ...
    if (!labSession) {
      throw new NotFoundException(`Lab session with ID ${labSessionId} not found`);
    }
  ...
  async bookComputer(labSessionId: string, computerId: string, userId: string): Promise<LabBooking> {
    // 1. Check if lab session and computer exist
    const labSession = await this.labSessionRepository.findOne({ where: { sessionId: labSessionId } });
    if (!labSession) {
      throw new NotFoundException(`Lab session with ID ${labSessionId} not found`);
    }

    const computer = await this.computerRepository.findOne({ where: { computerId: computerId } });
    if (!computer) {
    ...
    if (existingBooking) {
      throw new ConflictException(`Computer ${computerId} is already booked for lab session ${labSessionId}`);
    }
