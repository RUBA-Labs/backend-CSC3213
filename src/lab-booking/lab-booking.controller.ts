...
export class LabBookingController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Get booking status of computers for a lab session' })
  @Roles(Role.FIRST_YEAR_STUDENT, Role.ADMIN, Role.LAB_ALLOCATION_ADMIN, Role.DEVELOPER)
  async getLabSessionBookingStatus(@Param('labSessionId') labSessionId: string) {
  ...
  @ApiResponse({ status: 404, description: 'Lab session or computer not found.' })
  @ApiResponse({ status: 409, description: 'Computer already booked for this session.' })
  @Roles(Role.FIRST_YEAR_STUDENT, Role.ADMIN, Role.LAB_ALLOCATION_ADMIN, Role.DEVELOPER)
  async bookComputer(@Body() createLabBookingDto: CreateLabBookingDto, @Request() req: AuthenticatedRequest) {
    const userId = String(req.user.userId); // Cast userId to string
    const { labSessionId, computerId } = createLabBookingDto;
    return this.labBookingService.bookComputer(labSessionId, computerId, userId);
  }
}
